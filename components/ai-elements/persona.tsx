...
export const Persona: FC<PersonaProps> = memo(
  ({
    ...
    const stableCallbacks = useMemo(() => {
      // Use any type to avoid strict type issues with RiveParameters
      ...
      callbacks.onLoad = ((loadedRive: Parameters<NonNullable<RiveParameters["onLoad"]>>[0]) =>
        callbacksRef.current.onLoad?.(loadedRive));
      callbacks.onLoadError = ((err: Parameters<NonNullable<RiveParameters["onLoadError"]>>[0]) =>
        callbacksRef.current.onLoadError?.(err));
      callbacks.onPause = ((event: Parameters<NonNullable<RiveParameters["onPause"]>>[0]) =>
        callbacksRef.current.onPause?.(event));
      callbacks.onPlay = ((event: Parameters<NonNullable<RiveParameters["onPlay"]>>[0]) =>
        callbacksRef.current.onPlay?.(event));
      callbacks.onRiveReady = () => callbacksRef.current.onReady?.();
      callbacks.onStop = ((event: Parameters<NonNullable<RiveParameters["onStop"]>>[0]) =>
        callbacksRef.current.onStop?.(event));
      
      return callbacks;
    }, [source.source]);

    const { rive, RiveComponent } = useRive(stableCallbacks);
